...
/* --- Defines --- */
#define AZURE_PNP_MODEL_ID "dtmi:laboratorio1Iot:DispositivoLaboratorio1_6cv;1"
...
#define TELEMETRY_PROP_NAME_POWER                    "PotenciaGenerada"
#define TELEMETRY_PROP_NAME_TEMPERATURE              "TemperaturaDelPanel"
#define TELEMETRY_PROP_NAME_VOLTAGE                  "VoltajeDeSalida"
#define TELEMETRY_PROP_NAME_CURRENT                  "CorrienteDeSalida"
#define REPORTED_PROPERTY_ALERT_STATE                "EstadoDeAlerta"

#define POT_PIN                                      34
#define ALERT_LED_PIN                                2

static az_span COMMAND_NAME_CONTROL_ALERT_LED = AZ_SPAN_FROM_STR("ControlarLEDDeAlerta");
...
static bool alert_led_on = false;
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
  ...
  if (az_span_is_content_equal(command.command_name, COMMAND_NAME_CONTROL_ALERT_LED))
  {
    // IoT Central sends the Boolean request as true or false.
    // Accept both a raw Boolean and an object that contains true/false.
    const uint8_t* payload_ptr = az_span_ptr(command.payload);
    const int32_t payload_size = az_span_size(command.payload);
    bool requested_state = false;
    bool valid_payload = false;

    for (int32_t i = 0; i <= payload_size - 4; ++i)
    {
      if (memcmp(payload_ptr + i, "true", 4) == 0)
      {
    ...
    if (!valid_payload)
    {
      for (int32_t i = 0; i <= payload_size - 5; ++i)
      {
        if (memcmp(payload_ptr + i, "false", 5) == 0)
        {
    ...
    if (valid_payload)
    {
      alert_led_on = requested_state;
      digitalWrite(ALERT_LED_PIN, alert_led_on ? HIGH : LOW);
      LogInfo("ControlarLEDDeAlerta: %s", alert_led_on ? "ENCENDIDO" : "APAGADO");
      response_code = COMMAND_RESPONSE_CODE_ACCEPTED;
    }
    else
    {
      LogError("ControlarLEDDeAlerta recibio un payload no valido.");
    }
  }
  else
  {
    LogError("Command not recognized (%.*s).",
...
/* --- Sensors and actuators used by Laboratory 2 --- */
  ...
  size_t* payload_buffer_length)
{
  ...
  float temperatura_del_panel = dht.readTemperature();
  if (isnan(temperatura_del_panel))
  {
    temperatura_del_panel = 0.0f;
  }

  const int lectura_potenciometro = analogRead(POT_PIN);
  const float porcentaje = lectura_potenciometro / 4095.0f;

  // Synthetic but coherent solar-panel values derived from the potentiometer.
  const float voltaje_de_salida = 12.0f + (porcentaje * 12.0f);
  const float corriente_de_salida = 0.5f + (porcentaje * 7.5f);
  const float potencia_generada = voltaje_de_salida * corriente_de_salida;
  ...
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding PotenciaGenerada name.");
  rc = az_json_writer_append_double(&jw, potencia_generada, 1);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding PotenciaGenerada value.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_TEMPERATURE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding TemperaturaDelPanel name.");
  rc = az_json_writer_append_double(&jw, temperatura_del_panel, 1);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding TemperaturaDelPanel value.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_VOLTAGE));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding VoltajeDeSalida name.");
  rc = az_json_writer_append_double(&jw, voltaje_de_salida, 1);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding VoltajeDeSalida value.");

  rc = az_json_writer_append_property_name(&jw, AZ_SPAN_FROM_STR(TELEMETRY_PROP_NAME_CURRENT));
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding CorrienteDeSalida name.");
  rc = az_json_writer_append_double(&jw, corriente_de_salida, 2);
  EXIT_IF_AZ_FAILED(rc, RESULT_ERROR, "Failed adding CorrienteDeSalida value.");
  ...
  *payload_buffer_length = az_span_size(payload_buffer_span);
  ...
  LogInfo("Telemetry: %s", payload_buffer);
  return RESULT_OK;
}
